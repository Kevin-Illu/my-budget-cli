...
import { Result, TryCatch } from "@budget/core/result";
...
export class ExpenseRepository implements IExpenseRepository {
  ...
  async save(expense: CreateExpenseDTO): Promise<Result<ExpenseResponseDTO>> {
    return await TryCatch.run<ExpenseResponseDTO>(() =>
  ...
  async findAll(): Promise<Result<ExpenseResponseDTO[]>> {
    return await TryCatch.run<ExpenseResponseDTO[]>(() =>
      this.db.execute(async (s) => {
  ...
  async findById(id: number): Promise<Result<ExpenseResponseDTO | null>> {
    return await TryCatch.run<ExpenseResponseDTO>(() =>
      this.db.execute(async (s) => {
        const result =
